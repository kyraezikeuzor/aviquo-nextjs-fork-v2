...
export default function MarketplaceShare() {
  ...
  return (
    <main className="flex flex-col px-1/6 md:px-[10vw] lg:px-[10vw] gap-5">
      ...
      >
        <Icon icon="arrow-left" fillColor="#3981F6" /> Back to Marketplace
      </Link>
      <h1 className="tracking-tight text-lg md:text-xl lg:text-2xl">
        Submit a post to Marketplace
      </h1>
      <Card>
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              {interestList.map((item, index) => (
                <option
                  key={index}
                  className="text-xs md:text-sm lg:text-sm border-b border-[var(--clr-grey-300)] p-2"
                >
                  {item.name}
                </option>
              ))}
