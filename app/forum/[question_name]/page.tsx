...
export default function ForumPost({params: {question_name}}: Params) {
  ...
  return (
    <main className='flex flex-col px-[10vw] gap-5'>
        <Link href='/marketplace' className='hover:bg-[var(--clr-grey-200)] rounded-xl cursor-pointer text-xs md:text-sm lg:text-sm items-center text-[var(--clr-blue-400)] font-semibold flex gap-2'><Icon icon="arrow-left" fillColor="#3981F6"/> Back to Forum</Link>
        ...
        <h2 className='text-base md:text-xl lg:text-xl tracking-tight'>{postCommentsCount} Comments</h2>
        ...
        <div className='flex flex-col gap-5 cursor-pointer'>
                {posts.map((item,index)=>(
                    <div className='border-2 border-[var(--clr-grey-300)] p-4 rounded-xl flex flex-row gap-5'>
                        ...
                        <div className="flex flex-col gap-2">
                            <span className='inline-block flex gap-2 text-xs md:text-sm lg:text-sm'>@{item.username} • {item.date} <Tag type='tag'>{item.type}</Tag> </span>
