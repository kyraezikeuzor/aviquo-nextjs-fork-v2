  ...
  return (
    <nav className="px-[50px] md:px-[6vw] lg:px-[6vw] py-[1vh] flex flex-row items-center w-full">
      <ul className="w-full flex flex-row gap-2">
        <li>
          <div
            ...
            className={`${styles["mobile__sidebar__icon"]}`}
          >
        ...
        <Modal showModal={searchText != ""}>
          {modalSearchContent.map((item, index) => (
            <span
              key={index}
              className="flex flex-row items-center gap-4 hover:bg-[var(--clr-grey-200)]"
            >
              <FontAwesomeIcon icon={faMagnifyingGlass} className="w-4" />
              <Link
                href={`/ecs${getPath(item.name)}`}
      ...
      <div
        ...
        } border-2 border-[var(--clr-grey-300)] w-[50px] h-full p-2 bg-[#fff] fixed top-0 ${
          mobileSidebarClick ? "left-[0px]" : "left-[-200px]"
        } `}
      >
